...
<div class="m-app" wire:loading.class="is-loading" wire:target="openTable,closeOrder,saveOrder,clearTable">
    <div class="m-progress" wire:loading wire:target="openTable,closeOrder,saveOrder,clearTable,setTab,setPlaceFilter,addProduct,updateQuantity"></div>
    ...
    @if(! $this->activePlace)
        ...
        <header class="m-top">
            ...
            @if($tab === 'places')
                <div class="m-search">
                    ...
                    <input type="search" wire:model.live.debounce.250ms="placeSearch" placeholder="Stol yoki xona nomi...">
                    @if($placeSearch !== '')
                        <button type="button" wire:click="$set('placeSearch', '')" aria-label="Tozalash"><i class="mdi mdi-close"></i></button>
                    @endif
                </div>
                <div class="m-chips">
                    <button type="button" class="m-chip {{ $placeFilter === 'all' ? 'active' : '' }}" wire:click="setPlaceFilter('all')">
                        Hammasi <b>{{ $this->places->count() }}</b>
                    </button>
                    <button type="button" class="m-chip tone-free {{ $placeFilter === 'free' ? 'active' : '' }}" wire:click="setPlaceFilter('free')">
                        Bo'sh <b>{{ $free }}</b>
                    </button>
                    <button type="button" class="m-chip tone-busy {{ $placeFilter === 'busy' ? 'active' : '' }}" wire:click="setPlaceFilter('busy')">
                        Band <b>{{ $busy }}</b>
                    </button>
                </div>
            @endif
        ...
        <main class="m-main">
            @if($tab === 'places')
                ...
                @elseif($this->filteredPlaces->isEmpty())
                    <div class="m-empty">
                ...
                @else
                    <div class="m-places">
                        @foreach($this->filteredPlaces as $place)
    ...
    @else
        ...
        <header class="m-top">
            ...
            <div class="m-search">
                ...
                @if($search !== '')
                    <button type="button" wire:click="$set('search', '')" aria-label="Tozalash"><i class="mdi mdi-close"></i></button>
                @endif
            </div>

            <div class="m-chips scroll">
                <button type="button" class="m-chip {{ $selectedCategory === null && ! $onlyCart ? 'active' : '' }}"
                        wire:click="$set('selectedCategory', null); $set('onlyCart', false)">Barchasi</button>
                @if($kinds)
                    <button type="button" class="m-chip tone-cart {{ $onlyCart ? 'active' : '' }}" wire:click="$toggle('onlyCart')">
                        <i class="mdi mdi-cart-outline"></i> Savatda <b>{{ $kinds }}</b>
                    </button>
                @endif
                @foreach($this->categories as $category)
                    <button type="button" wire:key="mcat-{{ $category->id }}"
                            class="m-chip {{ (int) $selectedCategory === $category->id && ! $onlyCart ? 'active' : '' }}"
                            wire:click="$set('selectedCategory', {{ $category->id }}); $set('onlyCart', false)">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
        ...
        <main class="m-main has-cartbar">
            @if($this->mobileProducts->isEmpty())
                <div class="m-empty">
            ...
            @else
                <div class="m-products">
                    @foreach($this->mobileProducts as $product)
                        @php
                            $qty = $cart[$product->id]['quantity'] ?? 0;
                            $stock = (int) ($product->current_stock ?? 0);
                        @endphp
                        <div class="m-product {{ $qty ? 'in-cart' : '' }}" wire:key="mprod-{{ $product->id }}">
                            <button type="button" class="m-product-main" wire:click="addProduct({{ $product->id }})">
                                <span class="m-thumb">
                                    @if($product->imageUrl())
                                        <img src="{{ $product->imageUrl() }}" alt="" loading="lazy" onerror="this.remove()">
                                    @else
                                ...
                                <span class="m-product-body">
                                    ...
                                    <small>
                                        <b>{{ $fmt($product->sell_price) }}</b> so'm
                                        @if($product->discount > 0)<span class="m-off">-{{ $product->discount }}%</span>@endif
                                        <span class="m-stock {{ $stock > 0 ? '' : 'is-empty' }}">· {{ $stock }} dona</span>
                                    </small>
                                </span>
                            </button>
                            @if($qty)
                                <div class="m-stepper">
                                    <button type="button" wire:click="updateQuantity({{ $product->id }}, {{ $qty - 1 }})" aria-label="Kamaytirish"><i class="mdi mdi-minus"></i></button>
                                    <span>{{ $qty }}</span>
                                    <button type="button" wire:click="updateQuantity({{ $product->id }}, {{ $qty + 1 }})" aria-label="Ko'paytirish"><i class="mdi mdi-plus"></i></button>
                                </div>
                            @else
                                <button type="button" class="m-add" wire:click="addProduct({{ $product->id }})" aria-label="Qo'shish"><i class="mdi mdi-plus"></i></button>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        <div class="m-cartbar">
            @if($kinds)
                <button type="button" class="m-cartbar-info" wire:click="toggleCart">
                    <span class="m-cartbar-count">{{ $count }}</span>
                    <span class="m-cartbar-text">
                        <strong>{{ $fmt($this->total) }} so'm</strong>
                        <small>{{ $kinds }} xil · savatni ochish</small>
                    </span>
                    <i class="mdi mdi-chevron-up"></i>
                </button>
                <button type="button" class="m-btn primary" wire:click="saveOrder" wire:loading.attr="disabled">
        ...
        {{-- --------------------------------------------------------- savat --}}
        @if($showCart && ! $showCustomer)
            <div class="m-sheet-backdrop" wire:click="toggleCart"></div>
            <section class="m-sheet" role="dialog" aria-modal="true">
                <div class="m-sheet-handle"></div>
                <div class="m-sheet-head">
                    <h5><i class="mdi mdi-cart-outline"></i> Savat <span>{{ $kinds }}</span></h5>
                    <button type="button" class="m-icon-btn" wire:click="toggleCart" aria-label="Yopish"><i class="mdi mdi-close"></i></button>
                </div>

                <div class="m-sheet-body">
                    @if(empty($cart))
                        <div class="m-empty small"><i class="mdi mdi-cart-off"></i><p>Savat bo'sh.</p></div>
                    @else
                        <div class="m-cart-lines">
                            @foreach($cart as $line)
                                <div class="m-cart-line" wire:key="mline-{{ $line['product_id'] }}">
                                    <span class="m-cart-body">
                                        <strong>{{ $line['name'] }}</strong>
                                        <small>{{ $fmt($line['price']) }} so'm @if($line['discount'] > 0)<span class="m-off">-{{ $line['discount'] }}%</span>@endif</small>
                                    </span>
                                    <div class="m-stepper">
                                        <button type="button" wire:click="updateQuantity({{ $line['product_id'] }}, {{ $line['quantity'] - 1 }})"><i class="mdi {{ $line['quantity'] === 1 ? 'mdi-delete-outline' : 'mdi-minus' }}"></i></button>
                                        <span>{{ $line['quantity'] }}</span>
                                        <button type="button" wire:click="updateQuantity({{ $line['product_id'] }}, {{ $line['quantity'] + 1 }})"><i class="mdi mdi-plus"></i></button>
                                    </div>
                                    <span class="m-cart-sum">{{ $fmt($orderService->lineTotal($line)) }}</span>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="m-fields">
                            <label>
                                <span>Chegirma %</span>
                                <input type="number" min="0" max="100" inputmode="numeric" wire:model.live.debounce.500ms="discount" placeholder="0" onfocus="this.select()">
                            </label>
                            <label>
                                <span>Berilgan pul</span>
                                <input type="number" min="0" inputmode="numeric" wire:model.live.debounce.500ms="givenAmount" placeholder="0" onfocus="this.select()">
                            </label>
                        </div>

                        <div class="m-totals">
                            <div><span>Oraliq jami</span><b>{{ $fmt($this->subtotal) }}</b></div>
                            @if($discount > 0)
                                <div class="off"><span>Chegirma {{ $discount }}%</span><b>-{{ $fmt($this->subtotal - $this->total) }}</b></div>
                            @endif
                            <div class="grand"><span>To'lov</span><b>{{ $fmt($this->total) }} so'm</b></div>
                            @if((int) $givenAmount > 0)
                                <div class="change"><span>Qaytim</span><b>{{ $fmt($this->change) }}</b></div>
                            @endif
                        </div>
                    @endif
                </div>
                ...
                @if(! empty($cart))
                    <div class="m-sheet-foot">
                        ...
                        <button type="button" class="m-btn primary" wire:loading.attr="disabled"
                                x-on:click="mConfirm('Hisob yopilsinmi?', '{{ $fmt($this->total) }} so\'m to\'lov qabul qilinadi.', 'Ha, yopish').then(ok => ok && $wire.closeOrder())">
                        ...
                        <div class="m-list">
                            @forelse($this->customerResults as $customer)
                                <button type="button" wire:key="mcust-{{ $customer->id }}" class="m-row compact {{ $customerId === $customer->id ? 'is-selected' : '' }}"
                                        wire:click="selectCustomer({{ $customer->id }}); $set('showCustomer', false)">
                                    <span class="m-row-icon"><i class="mdi mdi-account-outline"></i></span>
